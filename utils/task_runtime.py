...
class TaskDeletedError(RuntimeError):
    """Raised when a background worker touches a task deleted by the user."""
...
def deleted_tasks_dir() -> Path:
    return runtime_root() / ".deleted"


def task_deleted_marker(task_id: str) -> Path:
    return deleted_tasks_dir() / f"{validate_runtime_id(task_id)}.json"


def is_task_deleted(task_id: str) -> bool:
    return task_deleted_marker(task_id).is_file()


def _assert_task_not_deleted(task_id: str) -> str:
    task_id = validate_runtime_id(task_id)
    if is_task_deleted(task_id):
        raise TaskDeletedError(f"任务已删除: {task_id}")
    return task_id
...
def register_task_workspace(task_id: str, workspace_id: str) -> str:
    task_id = _assert_task_not_deleted(task_id)
...
def rename_task(task_id: str, title: str) -> Dict[str, object]:
    """Rename a task for display without changing the immutable research query."""
    task_id = validate_runtime_id(task_id)
    title = str(title or "").strip()
    if not title:
        raise ValueError("任务名称不能为空")
    if len(title) > 120:
        raise ValueError("任务名称不能超过120个字符")

    root = task_dir(task_id)
    metadata_path = task_metadata_path(task_id)
    if not root.is_dir() or not metadata_path.exists():
...
def delete_task(task_id: str) -> None:
    """Delete a task immediately, even while background workers are still running.

    A tombstone is written before removing files so late worker writes cannot
    recreate the task after the user has deleted it.
    """
    task_id = validate_runtime_id(task_id)
    root = task_dir(task_id)
    marker = task_deleted_marker(task_id)
    if not root.is_dir() and not marker.exists():
        raise FileNotFoundError(task_id)

    deleted_tasks_dir().mkdir(parents=True, exist_ok=True)
    if not marker.exists():
        tmp = deleted_tasks_dir() / f".{task_id}.{uuid4().hex}.tmp"
        tmp.write_text(
            json.dumps(
                {"task_id": task_id, "deleted_at": time.time()},
                ensure_ascii=False,
                indent=2,
            ),
            encoding="utf-8",
    ...
    # On Unix this removes even open files; on Windows an open handle may keep
    # part of the directory alive temporarily. The tombstone remains the source
    # of truth either way and prevents resurrection.
...
def list_task_ids() -> list[str]:
    ...
    for entry in root.iterdir():
        if (
            ...
            and not is_task_deleted(entry.name)
        ):
...
def task_summary(task_id: str) -> Dict[str, object]:
    ...
    return {
        ...
        "title": str(metadata.get("title") or metadata.get("query") or ""),
