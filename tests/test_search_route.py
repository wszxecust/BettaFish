...
def test_task_can_be_renamed_without_changing_original_query(root_app):
    module, client, _ = root_app
    module.ensure_task(
        "task_rename",
        client_id="client_owner",
        workspace_id="workspace_owner",
        query="原始研究问题",
    )

    response = client.patch(
        "/api/tasks/task_rename",
        json={
            "workspace_id": "workspace_owner",
            "title": "自定义历史标题",
        },
    ...
    assert response.status_code == 200
    payload = response.get_json()
    assert payload["task"]["title"] == "自定义历史标题"
    assert payload["task"]["query"] == "原始研究问题"

    detail = client.get(
        "/api/tasks/task_rename?workspace_id=workspace_owner"
    ).get_json()["task"]
    assert detail["title"] == "自定义历史标题"
    assert detail["query"] == "原始研究问题"


def test_running_task_can_be_deleted_and_cannot_resurrect(root_app):
    module, client, tmp_path = root_app
    from utils.task_runtime import TaskDeletedError, claim_engine_run, ensure_task
    ...
    token, status = claim_engine_run("task_running_delete", "query")
    assert token
    assert status == "claimed"

    response = client.delete(
        "/api/tasks/task_running_delete",
        json={"workspace_id": "workspace_owner"},
    )

    assert response.status_code == 200
    assert response.get_json()["deleted"] is True
    assert not (tmp_path / "runtime" / "tasks" / "task_running_delete").exists()
    assert (
        tmp_path
        / "runtime"
        / "tasks"
        / ".deleted"
        / "task_running_delete.json"
    ).is_file()

    with pytest.raises(TaskDeletedError):
        ensure_task("task_running_delete", query="不应复活")


def test_completed_task_can_be_deleted(root_app):
    module, client, tmp_path = root_app
    from utils.task_runtime import claim_engine_run, finish_engine_run
    ...
    for engine in ("insight", "media", "query"):
        token, status = claim_engine_run("task_delete", engine)
        assert status == "claimed"
        finish_engine_run("task_delete", engine, token, success=True)

    task_dir = tmp_path / "runtime" / "tasks" / "task_delete"
    assert task_dir.is_dir()

    response = client.delete(
        "/api/tasks/task_delete",
        json={"workspace_id": "workspace_owner"},
    )
...
def test_task_mutations_require_workspace_ownership(root_app):
    module, client, _ = root_app
    module.ensure_task(
        "task_private_mutation",
        client_id="client_owner",
        workspace_id="workspace_owner",
        query="私有任务",
    )
    ...
    rename = client.patch(
        "/api/tasks/task_private_mutation",
        json={"workspace_id": "workspace_other", "title": "越权重命名"},
    )
