...
def main():
    ...
    if start_research:
        ...
        config = Settings(
            ...
            OUTPUT_DIR=str(task_output_dir(task_id, "insight"))
        )
...
def execute_research(query: str, config: Settings, task_id: str):
    """保证同一task的Insight Agent只执行一次；其他设备只订阅/查看。"""
    run_token, run_status = claim_engine_run(task_id, "insight")
    if run_status == "running":
        st.info("该任务的Insight Agent已在其他页面运行，本页面不会重复启动。")
        return False
    if run_status == "completed":
        st.info("该任务的Insight Agent已经完成，直接显示已保存结果。")
